...
type ProductItemProps = {
  item: Product;
  /** Chỉ hiện nút xem nhanh (ẩn thêm giỏ / yêu thích) */
  previewOnly?: boolean;
  /** Ẩn số lượng đánh giá (vẫn giữ sao nếu cần) */
...
const ProductItem = ({
  ...
  const { closePreviewModal } = usePreviewSlider();
  ...
  return (
    <div className="group">
      ...
      {item.categoryTitle && (
        <p className="text-sm text-blue mb-1.5">{item.categoryTitle}</p>
      )}
      ...
      <div className="flex items-center gap-2.5 mb-2">
        ...
        {!hideReviewCount && <p className="text-custom-sm">({item.reviews})</p>}
      </div>
