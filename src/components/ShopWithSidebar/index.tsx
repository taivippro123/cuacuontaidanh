...
function normalizeCategorySlug(
  ...
  const raw = (fromUrl ?? initial ?? "").trim();
...
const ShopWithSidebar = ({ initialCategorySlug }: ShopWithSidebarProps) => {
  const searchParams = useSearchParams();
  const categorySlugParam = normalizeCategorySlug(
    searchParams.get("category"),
    initialCategorySlug
  );

  const [products, setProducts] = useState<ProductData[]>([]);
  const [categories, setCategories] = useState<SidebarCategoryRow[]>([]);
  ...
  useEffect(() => {
    async function loadData() {
      ...
      const categoryCountById = productsData.reduce((acc, product) => {
      ...
      setCategories(
        categoriesData.map((category, index) => ({
          ...
          name: category.title,
          slug: category.slug,
          products: categoryCountById[category.id] || 0,
          isRefined: index === 0,
        }))
      );
  ...
  const displayedProducts = useMemo(() => {
    if (!categorySlugParam) return products;
    const needle = categorySlugParam.toLowerCase();
    const match = categories.find(
      (c) => (c.slug ?? "").toLowerCase() === needle
    );
    if (!loadingProducts && categories.length > 0 && !match) {
      return [];
    }
    if (!match) {
      return [];
  ...
  }, [products, categories, categorySlugParam, loadingProducts]);

  const totalCatalog = products.length;
  const showingCount = displayedProducts.length;
  const categoryTitleById = useMemo(
    () =>
      categories.reduce<Record<number, string>>((acc, c) => {
        acc[c.id] = c.name;
        return acc;
      }, {}),
    [categories]
  ...
  const totalPages = useMemo(() => {
    if (loadingProducts) return 1;
    const n = Math.ceil(showingCount / PAGE_SIZE);
    return n < 1 ? 1 : n;
  }, [loadingProducts, showingCount]);
  ...
  const rangeStart =
    showingCount === 0 ? 0 : (currentPage - 1) * PAGE_SIZE + 1;
  const rangeEnd = Math.min(currentPage * PAGE_SIZE, showingCount);
        ...
        <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
          <div className="flex gap-7.5">
              ...
              <form onSubmit={(e) => e.preventDefault()}>
                <div className="flex flex-col gap-6">
                  ...
                  <CategoryDropdown
                    ...
                    activeCategorySlug={categorySlugParam}
                  />
                </div>
              </form>
            </div>
            ...
            <div className="xl:max-w-[870px] w-full">
                    ...
                    <p>
                      ...
                      <span className="text-dark">
                        {loadingProducts
                          ...
                          : showingCount === 0
                            ? "0"
                            : `${rangeStart} trên ${showingCount}`}
                      </span>{" "}
                      sản phẩm
                      {categorySlugParam && !loadingProducts && (
                        <span className="text-gray-5"> (đã lọc)</span>
                      )}
                    </p>
                  </div>
              ...
              >
                {loadingProducts ? (
                  <div className="col-span-full flex min-h-[200px] items-center justify-center text-gray-5">
                    Đang tải sản phẩm...
                  </div>
                ) : (
                    ...
                    <ul className="flex items-center gap-0.5">
                      <li>
                        <button
                          ...
                          className="flex items-center justify-center w-8 h-9 ease-out duration-200 rounded-[3px] hover:text-white hover:bg-blue disabled:text-gray-4 disabled:hover:bg-transparent disabled:hover:text-current"
                        >
                          <svg
                            className="fill-current"
                            width="18"
                      ...
                      {paginationItems.map((item, idx) =>
                        item === "ellipsis" ? (
                          <li
                            key={`ellipsis-${idx}`}
                            className="flex py-1.5 px-2 text-dark-4 select-none"
                            aria-hidden
                          >
                            …
                          </li>
