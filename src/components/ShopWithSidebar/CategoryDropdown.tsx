...
const CategoryItem = ({
  ...
  return (
    <Link href={href} className={rowClass} scroll={false}>
      {inner}
    </Link>
  );
};
...
const CategoryDropdown = ({
  ...
  categories: SidebarCategoryRow[];
  activeCategorySlug: string | null;
}) => {
  const [toggleDropdown, setToggleDropdown] = useState(true);
  const active = activeCategorySlug?.toLowerCase() ?? null;
        ...
        {categories.map((category) => (
          <CategoryItem
            ...
            selected={
              !!active &&
              !!category.slug &&
              category.slug.toLowerCase() === active
            }
          />
        ))}
      </div>
