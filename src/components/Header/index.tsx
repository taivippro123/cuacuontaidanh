  ...
  return (
    ...
    >
      <div className="max-w-[1170px] mx-auto px-4 sm:px-7.5 xl:px-0">
        <div
          className={`relative flex items-center justify-between gap-4 ease-out duration-200 ${
        ...
        >
          ...
          <nav
            className={`z-50 max-xl:absolute max-xl:left-4 max-xl:right-4 max-xl:top-full max-xl:mt-2 max-xl:rounded-md max-xl:border max-xl:border-gray-3 max-xl:bg-white max-xl:p-4 max-xl:shadow-lg xl:mt-0 xl:rounded-none xl:border-0 xl:bg-transparent xl:p-0 xl:shadow-none ${
              navigationOpen ? "max-xl:block" : "max-xl:hidden"
            } xl:absolute xl:left-1/2 xl:top-1/2 xl:block xl:-translate-x-1/2 xl:-translate-y-1/2`}
          >
            <ul className="flex flex-col items-center gap-4 xl:flex-row xl:gap-10">
              {menuData.map((menuItem) => {
                const active =
                  menuItem.path && isNavActive(pathname, menuItem.path);
                return (
                  <li
                    key={menuItem.id}
                    className={`group relative before:h-[3px] before:bg-blue before:absolute before:left-0 before:top-0 before:rounded-b-[3px] before:ease-out before:duration-200 ${
                      active
                        ? "before:w-full"
                  ...
                  >
                    <Link
                      href={menuItem.path ?? "/"}
                      className={`text-custom-sm font-medium flex py-2 xl:py-0 ${
          ...
          <div className="relative z-10 flex flex-shrink-0 items-center gap-5 sm:gap-7.5">
